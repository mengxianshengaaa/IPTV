...
# 存储文件路径，使用通配符表示文件名以'网络收集'开头并以'.txt'结尾
file_path_pattern = '*网络收集.txt'
...
def process_file(file_path):
    ...
    # 临时输出文件路径
    temp_output_file_path = file_path + '.temp'
...
for filename in os.listdir(directory):
    if filename.startswith('网络收集') and filename.endswith('.txt'):
            ...
            with requests.get(url, stream=True, timeout=initial_timeout) as response:
                if 200 <= response.status_code <= 403:
                    ...
                    while time.time() - start_time < initial_timeout:
                        chunk = response.raw.read(256)  # 尝试下载 1KB 数据
                        if chunk:
                            return True  # 成功下载数据
        except requests.RequestException as e:
            print(f"请求异常: {e}")
...
def process_file(file_path_pattern):
    import os
            ...
            for line_number, line in enumerate(tqdm(lines, desc="检测中")):
                ...
                if len(parts) == 2 and parts[1]:
                    ...
                    try:
                        ...
                        print(f"检测 URL {channel_url} 时发生错误: {e}")
                        is_valid = False
...
def process_file(file_path_pattern):
    ...
    for filename in os.listdir('.'):
        if filename.startswith('网络收集') and filename.endswith('.txt'):
            ...
            ip_or_domain_to_lines = {}
            # 用于生成分类名的字母和数字计数器
            alphabet_counter = 0  # 字母计数器，从 0 开始
            number_counter = 1  # 数字计数器，从 1 开始
            # 读取原始文件内容
                ...
                for line in file:
                    ...
                    if match:
                        ...
                        if ip_or_domain not in ip_or_domain_to_lines:
                            ip_or_domain_to_lines[ip_or_domain] = []
                        ip_or_domain_to_lines[ip_or_domain].append(line)
            # 过滤掉小于 1000 字节的 IP 或域名段
            filtered_ip_or_domain_to_lines = {ip_or_domain: lines for ip_or_domain, lines in ip_or_domain_to_lines.items()
                                              if sum(len(line) for line in lines) >= 300}  # 过滤掉小于 1000 字节的 IP 或域名段
